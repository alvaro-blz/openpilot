...
def can_function(pm, speed, angle, idx, cruise_button=0, is_engaged=False):
  ...
  msg.append(packer.make_can_msg("GEAR_PACKET", 0, {"GEAR": 4}, idx))
  ...
  msg.append(packer.make_can_msg("EPS_STATUS", 0, {}, idx))
  msg.append(packer.make_can_msg("PCM_CRUISE", 0, {}, idx))
  msg.append(packer.make_can_msg("PCM_CRUISE_2", 0, {}, idx))
  #msg.append(packer.make_can_msg("SCM_FEEDBACK", 0, {"MAIN_ON": 1}, idx))
  msg.append(packer.make_can_msg("ACC_CONTROL", 0, {"SET_ME_X01": int(is_engaged), "SET_ME_X3":3}, idx))
  #print(msg)
  ...
  for k, v in FINGERPRINTS[CAR.PRIUS][0].items():
    if k not in done:
      msg.append([k, 0, b'\x00'*v, 0])
  msg.append(packer.make_can_msg("STEER_TORQUE_SENSOR", 0, {"STEER_ANGLE": angle_to_sangle(angle), "STEER_OVERRIDE":0}, idx))
